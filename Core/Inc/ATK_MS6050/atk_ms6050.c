...
#include "./ATK_MS6050/atk_ms6050_iic.h"
...
{
   if (HAL_I2C_Mem_Write(&hi2c1, addr << 1, reg, I2C_MEMADD_SIZE_8BIT, dat, len, 1000) == HAL_OK)
   {
...
{
   if (HAL_I2C_Mem_Read(&hi2c1, addr << 1, reg, I2C_MEMADD_SIZE_8BIT, dat, len, 1000) == HAL_OK)
   {
       return ATK_MS6050_EOK;
   }
   else
...
//    atk_ms6050_iic_init();                                                  /* 初始化IIC接口 */
    ...
    atk_ms6050_set_gyro_fsr(2);                                             /* 陀螺仪传感器，±2000dps */
